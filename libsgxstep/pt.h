...
#define RESERVED_SHIFT       51
#define RESERVED_MASK        (UINT64_C(0x1) << RESERVED_SHIFT)
...
#define CACHEABILITY(entry)		(((entry) & CD_MASK) >> CD_SHIFT)
#define MARK_CACHEABLE(entry) 	((entry) & ~CD_MASK)
#define MARK_NON_CACHEABLE(entry) 	((entry) | CD_MASK)

#define USER(entry) 			(((entry) & US_MASK) >> US_SHIFT)
#define MARK_USER(entry) 		((entry) | US_MASK)
#define MARK_SUPERVISOR(entry) 		((entry) & ~US_MASK)

#define RESERVED(entry) 		(((entry) & RESERVED_MASK) >> RESERVED_SHIFT)
#define MARK_RESERVED(entry) 		((entry) | RESERVED_MASK)
#define MARK_NON_RESERVED(entry)	((entry) & ~RESERVED_MASK)
